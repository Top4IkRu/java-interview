...
6. Как работает `HashSet`?
7. Как работает метод contains в `ArrayList`, `LinkedList`, `HashSet`?
8. В чём разница между `Iterable` и `Iterator`?
9. Что такое Iterator? В каких случаях нужно использовать iterator? И почему?
10. Что такое `HashMap`? Какие основные методы доступа и модификации данных в `HashMap`?
11. Может ли null быть ключом в `HashMap`?
12. Как работает `HashMap`?
13. Расскажите подробно, как работает метод put?
14. Что происходит при коллизии?
15. Что такое `HashSet`? Какая особенность у HashSet в отношении дубликатов и порядка элементов?
16. Какие коллекции из `Java Collections Framework` потокобезопасны (`thread-safe`) по умолчанию?
17. Что такое ConcurrentHashMap? В чем его отличие от `HashMap`?
18. Что такое синхронизированные (`synchronized`) коллекции? Какие есть синхронизированные реализации в Java?
19. Как можно сортировать элементы в коллекциях? Какой интерфейс следует использовать для сортировки?
